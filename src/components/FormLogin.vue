      ...
      <b-form @submit="onSubmit" @reset="onReset" v-if="show" id="formLogin">
        ...
        <div class="d-flex justify-content-center mt-3">
          <b-button type="submit" variant="primary" class="btn-login"
            >Login <font-awesome-icon icon="arrow-right" class="arrow"
          /></b-button>
        </div>
      ...
      this.$validator.validate().then((valid) => {
        ...
        if (valid) {
          this.$store.dispatch("login", this.form).then((res) => {
            if (res !== undefined) {
              if (res.status) {
                this.$store.dispatch("remember", this.remember);
                this.$awn.success(res.message);
                this.$router.push({ path: "/" });
              } else {
                this.$awn.warning(res.message);
              }
            } else {
              this.$awn.warning("Please verify your mail address before login");
            }
          });
        }
      });
